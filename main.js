/* main.js - Nganya Rides (module) - Works on GitHub Pages */
...
function resize(){ renderer.setSize(window.innerWidth, window.innerHeight - 164); camera.aspect = canvas.clientWidth / canvas.clientHeight; camera.updateProjectionMatrix(); }
window.addEventListener('resize', resize);

const scene = new THREE.Scene(); scene.background = new THREE.Color(0x071018); scene.fog = new THREE.FogExp2(0x071018, 0.002);
const camera = new THREE.PerspectiveCamera(60, window.innerWidth / (window.innerHeight - 164), 0.1, 1000);
...
// Stylized matatu (player)
const busGroup = new THREE.Group();
const busBody = new THREE.Mesh(new THREE.BoxGeometry(3.6,1.6,8), new THREE.MeshStandardMaterial({ color: state.busColor, metalness:0.1, roughness:0.4 }));
...
const bumper = new THREE.Mesh(new THREE.BoxGeometry(3.6,0.4,0.5), new THREE.MeshStandardMaterial({ color:0x0b0b0b }));
bumper.position.set(0,0.6,-4.25); busGroup.add(bumper);
const wheelGeo = new THREE.CylinderGeometry(0.45,0.45,0.6,16);
const wheelMat = new THREE.MeshStandardMaterial({ color:0x111111 });
const wheelPositions = [[-1.4,0.45,3], [1.4,0.45,3], [-1.4,0.45,-3], [1.4,0.45,-3]];
wheelPositions.forEach(p=>{ const w = new THREE.Mesh(wheelGeo, wheelMat); w.rotation.z = Math.PI/2; w.position.set(p[0],p[1],p[2]); busGroup.add(w); });
busGroup.position.set(0,0,0); scene.add(busGroup);
...
function animate(){
  ...
  traffic.children.forEach(c=>{
    c.userData.t += dt * 0.03;
    if(c.userData.t > 1) c.userData.t -= 1;
    const pos = paths[c.userData.pathIdx](c.userData.t);
    c.position.copy(pos);
    const next = paths[c.userData.pathIdx]((c.userData.t + 0.01) % 1);
    c.lookAt(next);
  });
  policeTimer += dt;
  if(policeTimer > 12 + Math.random()*30){ policeTimer = 0; triggerPolice(); }
  if(state.role === 'driver'){
    if(keys['w']||keys['arrowup']) speed += dt * 6;
    if(keys['s']||keys['arrowdown']) speed -= dt * 6;
    speed = Math.max(0, Math.min(25, speed));
    rotation = 0;
    if(keys['a']||keys['arrowleft']) rotation = 0.04;
    if(keys['d']||keys['arrowright']) rotation = -0.04;
    busGroup.rotation.y += rotation * speed * 0.02;
    busGroup.position.x += Math.sin(busGroup.rotation.y) * speed * dt;
    busGroup.position.z += Math.cos(busGroup.rotation.y) * speed * dt * -1;
    traffic.children.forEach(c=>{
      const dist = c.position.distanceTo(busGroup.position);
      if(dist < 3.5){
        log(t('bumped'));
    ...
    if(keys['c']){ conductorCollect(); keys['c'] = false; }
  }
...
updateLEDs(); updateDecal(); updateUI(); roleName.textContent = t(state.role); log(t('welcome')); animate();
