/* main.js v0.4 — Global THREE.js (for GitHub Pages, Conductor 1st-Person View) */
...
for (let i = 0; i < 4; i++) {
const z = -2.4 + i * 1.6;
const left = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.6, 1.4), seatMat);
left.position.set(-0.9, 0.6, z);
bus.add(left);
const right = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.6, 1.4), seatMat);
right.position.set(0.9, 0.6, z);
bus.add(right);
}

const windshield = new THREE.Mesh(
new THREE.BoxGeometry(3.6, 1.2, 0.2),
new THREE.MeshStandardMaterial({ color: 0x08101a, transparent: true, opacity: 0.6 })
);
windshield.position.set(0, 1.3, -3.6);
bus.add(windshield);

const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 2.0, 8), new THREE.MeshStandardMaterial({ color: 0xcccccc }));
pole.position.set(-0.1, 1.0, -0.5);
bus.add(pole);
...
const speed = 6;
const p0 = route[routeIndex];
const p1 = route[(routeIndex + 1) % route.length];
...
bus.position.x = pos.x;
bus.position.z = pos.z;
...
if (!p.paid) {
const pay = 20 + Math.floor(Math.random() * 40);
earned += pay;
p.paid = true;
}
});
totalEarned += earned;
moneyEl.textContent = Math.round(totalEarned);
...
overlay.innerHTML =
'<div class="panel"><h2>Trip Complete</h2><p>Total earned: KES ' +
totalEarned +
'</p><p>Passengers: ' +
passengers.length +
'</p><button id="close-summary">Close</button></div>';
...
// Camera controls
let yaw = 0, isPointerDown = false, last = { x: 0 };
...
last.x = e.clientX;
yaw -= dx * 0.003; // rotate head left/right
});

// --- Updated 1st-Person Conductor View ---
function updateCamera() {
// conductor seated near the door, looking back into bus
const conductorLocal = new THREE.Vector3(1.1, 1.5, -2.3);
const eyeWorld = conductorLocal.clone().applyMatrix4(bus.matrixWorld);
camera.position.lerp(eyeWorld, 0.2);

const lookAtLocal = new THREE.Vector3(0, 1.3, 1.0);
const lookWorld = lookAtLocal.clone().applyMatrix4(bus.matrixWorld);
